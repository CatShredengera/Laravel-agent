...
// Ported from jenssegers/agent's tests/AgentTest.php — the golden set of
// user-agent strings the original package was validated against. Passing
// this suite is the compatibility bar for this drop-in replacement.
$operatingSystems = [
...
test('languages', function () {
    ...
    expect($agent->languages())->toBe(['nl-nl', 'nl', 'en-us', 'en']);
});

test('languages sorted', function () {
    $agent = new Agent;
    $agent->setHttpHeaders([
        'HTTP_ACCEPT_LANGUAGE' => 'en;q=0.4,en-US,nl;q=0.6',
    ]);

    expect($agent->languages())->toBe(['en-us', 'nl', 'en']);
});

test('operating systems', function () use ($operatingSystems) {
    $agent = new Agent;
...
test('browsers', function () use ($browsers) {
    $agent = new Agent;
...
test('robots', function () use ($robots) {
    $agent = new Agent;
...
test('robot should return false', function () {
    $agent = new Agent;

    expect($agent->robot())->toBeFalse();
});

test('call should throw BadMethodCallException', function () {
    $agent = new Agent;

    $agent->invalidMethod();
})->throws(BadMethodCallException::class);

test('mobile devices', function () use ($mobileDevices) {
    $agent = new Agent;
...
test('desktop devices', function () use ($desktopDevices) {
    $agent = new Agent;
...
test('versions', function () use ($browserVersions, $operatingSystemVersions, $browsers) {
    $agent = new Agent;

    foreach ($browserVersions as $ua => $version) {
        $agent->setUserAgent($ua);
        $browser = $agent->browser();
        expect($agent->version($browser))->toBe($version);
    }

    foreach ($operatingSystemVersions as $ua => $version) {
        $agent->setUserAgent($ua);
        $platform = $agent->platform();
        expect($agent->version($platform))->toBe($version);
    }

    foreach ($browsers as $browser) {
        $agent->setUserAgent('FAKE');
        expect($agent->version($browser))->toBeFalse();
    }
});

test('is methods', function () use ($desktops, $phones, $robots, $mobileDevices, $desktopDevices) {
    $agent = new Agent;
